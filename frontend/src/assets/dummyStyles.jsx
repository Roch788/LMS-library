...
// Admin Catalog Page Styles
export const adminCatalogPageStyles = {
  pageContainer: "space-y-6",
  mainSection: "rounded-[26px] border border-white/70 bg-white/75 p-5 shadow-[0_25px_60px_rgba(20,35,29,0.18)] backdrop-blur-[18px] sm:rounded-[30px] sm:p-6",
  headerFlex: "flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between",
  title: "font-display text-2xl font-semibold text-library-ink sm:text-3xl",
  subtitle: "mt-1 text-sm text-library-ink/65",
  addButton: "inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-library-panel px-5 py-3 text-sm font-semibold text-library-paper sm:w-auto",

  // tabs
  tabsContainer: "flex flex-wrap gap-3 border-b border-library-ink/10 pb-4",
  tabButton: "rounded-full px-5 py-2.5 text-sm font-semibold transition",
  tabActive: "bg-library-panel text-library-paper shadow-md",
  tabInactive: "bg-white/80 text-library-ink hover:bg-library-paper",

  // filters
  filtersContainer: "mt-5 grid gap-4 rounded-3xl border border-library-ink/10 bg-library-paper/70 p-4 lg:grid-cols-3",
  filterLabel: "block",
  filterLabelSpan: "mb-2 block text-xs font-bold uppercase tracking-[0.2em] text-library-ink/45",
  searchWrapper: "flex items-center gap-2 rounded-2xl border border-library-ink/10 bg-white/85 px-4 py-3",
  searchIcon: "text-library-ink/55",
  ...
  // books grid
  booksGrid: "mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3",
  bookCard: "flex flex-col justify-between rounded-[26px] border border-library-ink/10 bg-white/85 p-5 shadow-sm",
  bookCardHeader: "flex items-start justify-between gap-3",
  categoryBadge: "inline-flex rounded-full bg-library-paper px-3 py-1 text-xs font-bold text-library-ink/70",
  availabilityBadge: "inline-flex rounded-full px-2.5 py-0.5 text-xs font-bold",
  bookTitle: "mt-3 font-display text-xl font-semibold leading-snug text-library-ink",
  bookAuthor: "mt-1 text-sm text-library-ink/65",
  bookFooter: "mt-5 border-t border-library-ink/8 pt-3 text-xs text-library-ink/60 flex items-center justify-between",
  bookActions: "mt-3 flex gap-2",
  editButton: "rounded-xl border border-library-ink/10 bg-white/80 px-3 py-2 text-xs font-semibold text-library-ink transition hover:bg-library-paper",
  deleteButton: "rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700 transition hover:bg-rose-100",

  // add/edit form modal
  modalOverlay: "fixed inset-0 z-50 flex items-center justify-center bg-library-ink/40 backdrop-blur-sm p-4",
  modalContent: "w-full max-w-2xl rounded-[26px] border border-white/70 bg-white p-6 shadow-2xl max-h-[90vh] overflow-y-auto",
  modalTitle: "font-display text-2xl font-semibold text-library-ink",
  modalForm: "mt-5 grid gap-4 md:grid-cols-2",
  modalLabel: "block",
  modalLabelSpan: "mb-2 block text-xs font-bold uppercase tracking-[0.2em] text-library-ink/45",
  modalInput: "w-full rounded-2xl border border-library-ink/10 bg-library-paper/80 px-4 py-3 outline-none",
  modalSelect: "w-full rounded-2xl border border-library-ink/10 bg-library-paper/80 px-4 py-3 outline-none",
  modalTextarea: "w-full rounded-2xl border border-library-ink/10 bg-library-paper/80 px-4 py-3 outline-none min-h-[80px] resize-y",
  modalButtons: "mt-5 flex flex-col gap-3 sm:flex-row md:col-span-2",
  modalSubmitButton: "flex-1 rounded-2xl bg-library-panel px-5 py-3 text-sm font-semibold text-library-paper",
  modalCancelButton: "flex-1 rounded-2xl border border-library-ink/10 bg-white px-5 py-3 text-sm font-semibold text-library-ink",

  // requests section
  requestCard: "rounded-[22px] border border-library-ink/10 bg-[linear-gradient(145deg,color-mix(in_srgb,var(--color-library-paper)_90%,white),color-mix(in_srgb,white_96%,transparent))] p-4 shadow-sm",
  requestHeader: "flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between",
  requestTitle: "font-semibold text-library-ink",
  requestDetail: "mt-1 text-sm text-library-ink/60",
  requestStatusBadge: "inline-flex rounded-full px-3 py-1 text-xs font-bold",
  requestActions: "mt-3 flex gap-2",
  approveButton: "rounded-xl bg-emerald-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-emerald-700",
  rejectButton: "rounded-xl bg-rose-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-rose-700",
